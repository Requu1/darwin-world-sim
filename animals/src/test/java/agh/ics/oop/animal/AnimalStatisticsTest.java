...
class AnimalStatisticsTest {
    ...
    @Test
    void updateStatsGenomeSequencePointsToAnimalGenomeSequence() {
        //given
        Animal animal = new Animal(new Vector2d(0, 0), 20, new Genome(new ArrayList<>(List.of(1, 2, 3))));
        AnimalStatistics stats = animal.getStats();

        //when
        stats.updateStats(AnimalStatisticsData.UPDATE_GENOME_SEQUENCE, animal);

        //then
        assertSame(animal.getGenome().getSequence(), stats.getGenomeSequence());
    }
}
